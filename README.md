...
| 元数据过滤 | 支持按 `category` 过滤，适合配合 `data/sample_large_en.json` 演示 |
| 数据管理 | 记录数查询、单条删除、清空集合、本地 `data/user_data.json` 备份 |
| 运维辅助 | `/health` 健康检查、Makefile 常用命令、三库性能基准测试 |

边界也要明确：这是学习和对比项目，不是生产级服务。Web UI 和 API 目前没有鉴权、多用户隔离、审计日志和限流；如果部署到服务器，只建议放在内网、VPN 或 Nginx Basic Auth 后面。

> ⚠️ **注意元数据写入方式**：命令行 `python -m src.ingest your.json` 会保留 JSON 里的 `category` 等额外字段；Web UI 和 `POST /api/ingest` / `POST /api/upload` 目前只写入文本并自动生成 ID，不保留上传文件里的额外字段。需要演示分类过滤时，优先用 `make ingest-large` 或命令行入库带 `category` 的 JSON。
...
写入成功后，文本会同时追加到 `data/user_data.json`，方便向量库清空后重新入库。
...
当前上传接口只读取 `text` 字段并自动分配新 ID；`category` 等额外字段不会通过该接口保留。需要保留元数据时，用命令行 `python -m src.ingest your.json`。
...
curl -X POST http://localhost:8888/api/search \
  -H "Content-Type: application/json" \
  -d '{"query": "法国著名地标", "limit": 5, "category": "geography"}'
```
...
| `category` | `string \| null` | `null` | 可选分类过滤；只返回该分类下的结果 |
...
通过 Web UI 或 REST API 写入的自定义文本会追加到 `data/user_data.json`（本地备份，已加入 `.gitignore`，不会进仓库）。
