...
边界也要明确：当前服务已经具备数据库持久化、索引重建、健康检查、基础审计日志和 REST API Key 鉴权，但还没有多租户、限流和告警。Web UI 仍建议放在内网、VPN、网关鉴权或 Nginx Basic Auth 后面。
...
> - REST API 默认启用 `X-API-Key` 鉴权，密钥从 `.env` 的 `API_KEY` 读取；`/health` 和少量只读 Web UI 辅助接口保持公开。
> - Web UI 目前无登录态，建议加 Nginx Basic Auth 或放到内网 / VPN 后面。
...
**Q: 代码里为什么没有 API Key？**
A: REST API Key 在 `.env` 配置：`AUTH_ENABLED=1`、`API_KEY=...`、`API_KEY_HEADER=X-API-Key`。本地 Docker 起的向量服务默认无鉴权，正式部署必须放在内网或网关后面，并开启 PostgreSQL 强密码、网关鉴权和访问日志。
