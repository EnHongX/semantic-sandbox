...
```bash
cd ..
docker compose --profile milvus up -d postgres milvus-etcd milvus-minio milvus attu
cd milvus-service
make init-db
```
...
**5. 三个容器缺一不可**
Milvus standalone 需要 etcd（元数据）+ minio（存储）+ milvus（主进程）同时健康。`docker compose -f ../docker-compose.yml ps` 任何一个 Exiting 都会导致 Milvus 功能异常。
...
| <http://localhost:8890/logs> | 日志页（审计 / 搜索 / 错误 / 导入任务） |
...
- **standalone vs cluster**：当前服务是 standalone（单机）。生产推荐 cluster 模式，写读分离、能水平扩展
