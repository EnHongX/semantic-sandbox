"""Web UI + REST API for Milvus service.
...
app = FastAPI(
    title="Milvus Service API",
    description=(
        "向量数据库写入 & 语义搜索接口。\n\n"
        "- `POST /api/ingest`：文本 → PostgreSQL 元数据主库 → Milvus 向量索引\n"
...
def _fmt_exc(exc: Exception) -> str:
    ...
    if any(k in msg.lower() for k in _CONN_KEYWORDS):
        return "向量数据库未连接，请先在项目根目录执行 docker compose --profile milvus up -d postgres milvus-etcd milvus-minio milvus attu（Milvus 首次启动约需 30 秒）"
    return msg
...
def _search_results(client: MilvusClient, *, query: str, limit: int, filters: dict) -> list[dict]:
...
async def health():
    ...
    try:
        ...
        return {"status": "ok", "db": "milvus", "metadata_store": metadata_state["store"]}
    except Exception as exc:
...
@app.post(
    ...
    summary="写入文本到向量库",
    description=(
        "将 `texts` 列表中的每条文本向量化后 upsert 到 Milvus，同时写入 PostgreSQL 元数据主库。\n\n"
...
@app.post(
    ...
    summary="语义搜索",
    description=(
        "将 `query` 向量化后在 Milvus 里做近邻检索，并支持多分类、标签、时间范围过滤。\n\n"
        "`score` 为 COSINE 相似度，越接近 1 越相似；返回值包含匹配片段、高亮词和 score 解释。"
    ),
    tags=["语义搜索"],
)
...
async def api_count() -> dict:
    ...
    try:
        if COLLECTION_NAME not in client.list_collections():
            return {"count": 0}
        stats = client.get_collection_stats(collection_name=COLLECTION_NAME)
        return {"count": int(stats.get("row_count", 0))}
    except Exception:
        return {"count": 0}
    finally:
        client.close()
...
@app.delete(
    ...
    description="删除并重建当前 Milvus collection，同时清空 PostgreSQL 文档元数据。",
    tags=["数据写入"],
)
...
async def api_batch_delete_documents(req: BatchDocumentRequest) -> dict:
    ...
    client = _get_client()
    try:
        client.delete(collection_name=COLLECTION_NAME, ids=record_ids)
...
def _reindex_all(client: MilvusClient) -> int:
    docs = load_documents()
    try:
        client.drop_collection(collection_name=COLLECTION_NAME)
    except Exception:
