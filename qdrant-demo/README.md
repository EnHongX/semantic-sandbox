...
3. **`data/user_data.json`**（本地 JSON 文件）：兼容镜像，格式与 `sample_en.json` 接近；Qdrant 数据清空后可用 `python -m src.ingest ../data/user_data.json` 重新入库

> 为什么双写？ 向量库适合检索，但不擅长原始数据备份。`data/documents.json` 是主元数据文件，查询 / 去重优先读它；`data/user_data.json` 主要保留给兼容和重建场景使用。
