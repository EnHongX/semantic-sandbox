...
app = FastAPI(
    ...
    description=(
        "向量数据库写入 & 语义搜索接口。\n\n"
        "- `POST /api/ingest`：文本 → 向量 → 写入 Weaviate，同时写入 `data/documents.json` 并镜像到 `data/user_data.json`\n"
...
@app.post(
    ...
    summary="写入文本到向量库",
    description=(
        "将 `texts` 列表中的每条文本向量化后写入 Weaviate，同时写入 `data/documents.json` 并镜像到 `data/user_data.json`。\n\n"
...
@app.delete(
    ...
    description="删除并重建当前 Weaviate collection，同时清空 `data/documents.json` 和 `data/user_data.json`。",
    tags=["数据写入"],
)
