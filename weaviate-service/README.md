...
用 [Weaviate](https://weaviate.io/) 实现的语义检索服务后端。Weaviate 本身支持内置向量化（丢文本进去自己算向量），为了和另外两个服务对齐，我们这里**关掉**它内置的向量化模块，统一用 `sentence-transformers` 在 Python 端算好向量再写入。
...
# 如果容器没起来，查日志
docker compose -f ../docker-compose.yml logs weaviate
```
...
| 地址 | 用途 |
|---|---|
| <http://localhost:8889> | 搜索页面（多分类 / 标签 / 时间范围过滤，展示片段、高亮和 score 解释） |
...
> 与命令行 `ingest.py` 不同，Web API 写入是**增量的**（不会删除已有数据）。Web/API 写入支持 `category`、`tags`、`source`；会按 `document_id` / 规范化文本生成的 `text_hash` 做幂等判断。重复提交不会再次入库，而是返回已存在记录明细。
...
> 查询 / 去重 / 重建索引都以 PostgreSQL `documents` 表为准。
