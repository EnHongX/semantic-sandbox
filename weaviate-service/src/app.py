"""Web UI + REST API for Weaviate service.
...
app = FastAPI(
    title="Weaviate Service API",
    description=(
        "向量数据库写入 & 语义搜索接口。\n\n"
        "- `POST /api/ingest`：文本 → PostgreSQL 元数据主库 → Weaviate 向量索引\n"
...
def _fmt_exc(exc: Exception) -> str:
    ...
    if any(k in msg.lower() for k in _CONN_KEYWORDS):
        return "向量数据库未连接，请先在项目根目录执行 docker compose --profile weaviate up -d postgres weaviate"
    return msg
...
async def health():
    ...
    try:
        ...
        return {"status": "ok", "db": "weaviate", "metadata_store": metadata_state["store"]}
    except Exception as exc:
...
@app.post(
    ...
    summary="写入文本到向量库",
    description=(
        "将 `texts` 列表中的每条文本向量化后写入 Weaviate，同时写入 PostgreSQL 元数据主库。\n\n"
...
@app.post(
    ...
    summary="上传文件批量写入向量库",
    description=(
        "上传 JSON（`[{\"text\":\"...\"}]`）或 CSV（首行含 `text` 列）文件，批量向量化写入 Weaviate；"
